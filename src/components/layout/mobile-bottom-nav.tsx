...
export function MobileBottomNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 z-50 flex h-20 w-full items-center justify-around border-t border-outline-variant/30 bg-surface px-4 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_20px_rgba(0,0,0,0.04)] md:hidden"
      aria-label="Primary"
    >
      {items.map((item) => {
        const active =
